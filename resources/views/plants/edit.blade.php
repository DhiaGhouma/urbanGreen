...
@section('title', 'Modifier {{ $plant->name }} - UrbanGreen')
...
<div class="page-header">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Modifier {{ $plant->name }}</li>
        </ol>
    </nav>
    <h1><i class="fas fa-edit me-2"></i>Modifier la Plante</h1>
</div>
...
<div class="row justify-content-center">
    <div class="col-md-10">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('greenspaces.plants.update', [$greenspace, $plant]) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number"
                                   ...
                                   id="quantity"
                                   name="quantity"
                                   value="{{ old('quantity', $plant->quantity ?? 1) }}">
                            @error('quantity')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date"
                                   ...
                                   id="planted_at"
                                   name="planted_at"
                                   value="{{ old('planted_at', $plant->planted_at ? $plant->planted_at->format('Y-m-d') : '') }}">
                            @error('planted_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror"
                                ...
                                <option value="en vie" {{ old('status', $plant->status) == 'en vie' ? 'selected' : '' }}>En vie</option>
                                <option value="malade" {{ old('status', $plant->status) == 'malade' ? 'selected' : '' }}>Malade</option>
                                <option value="abattu" {{ old('status', $plant->status) == 'abattu' ? 'selected' : '' }}>Abattu</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text"
                                   ...
                                   id="maintenance"
                                   name="maintenance"
                                   value="{{ old('maintenance', $plant->maintenance) }}"
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror"
                                      ...
                                      name="notes"
                                      rows="3"
                                      placeholder="Observations particulières, historique, etc.">{{ old('notes', $plant->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
